	...
	<div class="blogUser-con">
		...
		<blockquote class="layui-elem-quote not_border_left">
			<form class="layui-form" action="">
				...
				<button type="button" class="layui-btn layui-bg-blue" id="addartType" lay-event="addartType" lay-filter="addartType" style="margin-left: 10px;">新增学期</button>
			</form>
		</blockquote>
		<!-- 条件筛选框End -->
				...
				<form class="layui-form" action="">
					...
					<div class="layui-form-item">
						...
						<div class="layui-input-block">
							<input type="text" name="addsemesterName" id="addsemesterName"
								lay-verify="addsemesterName" autocomplete="off" placeholder="请输入标题" class="layui-input">
						</div>
					</div>
					<div class="layui-form-item">
	...
	layui.use([ 'table', 'form', 'layer', 'laydate', 'laytpl', 'element' ], function() {
		...
		$("#addartType").click(function(){
			...
			layer.open({
				...
				btn : [ '保存', '返回' ],
				yes : function() {
					var addsemesterName = $("#addsemesterName").val().trim();
					var startdate = $("#test").val().trim();
					

					if(addsemesterName == "") {
						layer.tips('不能为空', '#addsemesterName');
						return;
					} 
					
					$.ajax({
						type : 'get',
						url : '../curr/addsemester?semesterName='+addsemesterName+'&startdate='+startdate,
						datatype : 'json',
				...
				case 'del':
					layer.confirm('确定要删除么？', {
					  ...
					  icon:3
						...
						$.ajax({
			        		type: 'get',
			        		url: "../del/delsemester?id="+data.semesterid,
			        		dataType: 'json',
			        		success:function(data){
			        			if(data.code == 0){
			        				layer.confirm(data.msg, {
